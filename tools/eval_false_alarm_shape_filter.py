...
def reject_bird_like(feat, conf):
    if feat is None:
    ...
    ecc = feat["eccentricity"] > 0.93

    if conf < 0.45 and ((elongated and sparse) or (ecc and irregular)):
        return True, "bird_like_lowconf"
    if (elongated and sparse and irregular) or (ecc and irregular):
    ...
    for idx, img_path in enumerate(files, 1):
        ...
        if r.boxes is not None and len(r.boxes) > 0:
            ...
            for i in range(len(xyxy)):
                ...
                # chỉ xét prediction Drone cho false alarm suppression
                if pred_cls != 0:
                    continue
                ...
                reject, reason = reject_bird_like(feat, conf)
                keep = not reject
                if keep:
