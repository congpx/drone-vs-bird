...
def main(args):
    ...
    for idx, img_path in enumerate(files, 1):
        ...
        if r.boxes is not None and len(r.boxes) > 0:
            ...
            for i in range(len(xyxy)):
                ...
                pred_bird_fp = int(clf.predict(x)[0]) == 1

                keep = not pred_bird_fp
                if keep:
    ...
    with (outdir / "bird_ml_filter_debug.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    print(" -", outdir / "bird_ml_filter_debug.csv")
