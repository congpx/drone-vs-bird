...
def resolve_dirs(data_yaml, split="test"):
    ...
    split_key = split
    if split_key == "valid" and "valid" not in data and "val" in data:
        split_key = "val"
...
def mask_features(poly):
    ...
    compactness = 0.0 if peri <= 1e-6 else area / (peri * peri + 1e-9)
    ...
    approx = cv2.approxPolyDP(pts.astype(np.int32), 0.02 * peri, True)
...
def main(args):
    ...
    fieldnames = [
        ...
        "label_bird_fp", "label_true_drone", "label_other_fp"
    ]
    ...
    for idx, img_path in enumerate(files, 1):
        ...
        for i in range(len(xyxy)):
            ...
            label_true_drone = 1 if best_iou_drone >= args.iou_match else 0
            label_bird_fp = 1 if (label_true_drone == 0 and best_iou_bird >= args.iou_match) else 0
            label_other_fp = 1 if (label_true_drone == 0 and label_bird_fp == 0) else 0
            ...
            rows.append({
                ...
                "rectangularity": feat["rectangularity"],
                ...
                "label_bird_fp": label_bird_fp,
                "label_true_drone": label_true_drone,
                "label_other_fp": label_other_fp,
            })
